...
- Aligned DeepAgents auth preflight with the CLI's credential sources and harnessctl agent env, including DeepAgents' `/auth` store, shell environment, project `.env`, `~/.deepagents/.env`, harnessctl `.env` files, and `~/.harnessctl/agents/deepagents.yaml`.
